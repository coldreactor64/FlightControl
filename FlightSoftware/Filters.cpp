...
static float ypr[3]; //ypr[0] = yaw ypr[1]= pitch ypr[0] = roll
...
void MadgwickQuaternionUpdate(int16_t Iax, int16_t Iay, int16_t Iaz, int16_t Igx, int16_t Igy, int16_t Igz, float mx, float my, float mz, float deltat)
{
  ...
  ypr[1] *= 180.0f / PI;
  ypr[0] *= 180.0f / PI; 
  ypr[0] -= 13.8; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
  ypr[2] *= 180.0f / PI;
}
